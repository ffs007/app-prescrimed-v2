...
| 0.6 | CI mínimo no GitHub Actions: `tsc`, `vitest`, `build`, smoke Playwright | PR só passa verde | — |
| 0.7 | **Religar `DocumentsToGenerateDialog` (Q6) como etapa de revisão pré-emissão**, alimentando a emissão que já existe. Não como segundo emissor. Desenho na seção 5, Q6 | Perfil de assinatura (`assinatura_perfis`), Anexo IV e o gate `exigir_revisao_final_concluida` passam a valer no PrintArea; nenhuma saída sem `persistEmission`; um só classificador de receita | — |
...
**Desenho adotado (passo 0.7):** aproveitar do diálogo só o que agrega (perfil de assinatura, Anexo IV, gate `exigir_revisao_final_concluida`) como etapa de revisão antes do `PrintArea`, usando os grupos regulatórios que já existem. O `PrintArea` passa a ler o perfil escolhido (com o `signature-config` local como fallback). Depois disso, `DocumentPreviewDialog` e o caminho `renderDocument`/`saveGeneratedDocument` saem, ou o teste de integração `prescription-flow.test.ts` (que hoje os usa) é migrado. Depende da refatoração do Dashboard (2.0) para ficar limpo, mas o gate e o perfil podem entrar antes.
