...
| 0.2 | ✅ **CONCLUÍDO (2026-09-21).** Migrations aplicadas em `zwwalaioamxcvxbihxlr`: `storage_buckets_documentos` (buckets `documentos-pdf` privado + `protocolos-usuario`), `patologias_tabelas_e_rpcs_admin` (4 tabelas `patologia_*`/`patologias_usuario` + RPCs `promover_stg_*`/`aprovar_lote` com guard admin), `ia_credenciais_mascaradas` (view) | Enum, buckets, 4 tabelas `patologia_*`, view mascarada e guard de admin no ar; RPCs admin negam não-admin |
| 0.3 | ✅ **CONCLUÍDO (2026-09-21).** As 7 functions do commit `f676149` (ai-assist, clinical-ai, create-checkout, create-portal-session, iv-extract, public-document-link, smart-input-extract) deployadas via MCP, `verify_jwt` preservado por função | Nenhuma resposta devolve texto de erro do provedor; `/d/:token` mostra o PDF |
| 0.4 | ✅ **CONCLUÍDO (2026-09-21).** `SELECT ... WHERE relkind='r' AND NOT relrowsecurity` em `public`: **vazio**. Checagem extra (RLS ligado mas zero `pg_policies`, que travaria tudo por esquecimento): também **vazio** | Lista vazia ou justificada |
