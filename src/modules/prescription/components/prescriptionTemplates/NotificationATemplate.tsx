/**
 * Template — Notificação de Receita A (amarela). Listas A1/A2/A3.
 *
 * Características:
 *   - A4 retrato, 1 via (retida na farmácia)
 *   - Faixa amarela no topo identificando o tipo
 *   - Campo "Nº da Notificação" em branco para preenchimento manual
 *     (talonário oficial fornecido pela vigilância sanitária)
 *   - Limite de 1 substância por notificação (já garantido pelo motor)
 *   - Identificação completa: emitente, paciente, comprador, fornecedor
 *   - Aviso explícito de obrigatoriedade do talonário oficial
 */
import { AlertTriangle } from "lucide-react";
import PatientLine from "./PatientLine";
import { todayBR, type PrescriptionTemplateProps } from "./types";

const NotificationATemplate = ({
  meds,
  patientName,
  isPediatric,
  isPregnant,
  ...
  return (
    <div className="prescription-template-notif-a">
      {/* Faixa amarela identificadora */}
      <div className="h-2 -mx-10 -mt-10 mb-3 bg-receipt-yellow print:-mx-[10mm] print:-mt-[10mm]" />
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        {/* Bloco do número da notificação (campo em branco) */}
      ...
      <div className="text-center mb-3">
        ...
        <p className="text-[10px] text-muted-foreground italic">
          Talonário oficial amarelo numerado · 1 via retida na farmácia
        </p>
      </div>
      ...
      {/* Prescrição (1 substância apenas, garantido pelo motor) */}
      ...
      {/* Aviso de talonário oficial */}
      <div className="mt-3 p-2 border border-receipt-yellow/50 rounded bg-receipt-yellow-soft print:bg-receipt-yellow-soft">
        <p className="text-[9px] text-foreground flex items-start gap-1.5 leading-snug">
          <AlertTriangle className="h-3 w-3 text-receipt-yellow shrink-0 mt-0.5" />
          <span>
            <strong>Atenção:</strong> Este documento serve como rascunho/referência. A dispensação
            só é válida quando transcrita no talonário oficial amarelo numerado, fornecido pela
            vigilância sanitária local. Validade: {validityDays} dias.
          </span>
        </p>
      </div>
    </div>
...
export default NotificationATemplate;
